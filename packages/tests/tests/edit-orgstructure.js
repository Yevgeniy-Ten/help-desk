...
When(
  "я  жму  ссылку {string} с аттрибутом {string} и с текстом {string}",
  (a, title, text) => {
    I.wait(1);
    const element = locate(a).withAttr({ title: title }).withText(text);
    I.click(element);
    I.wait(1);
  }
);
// Дравер оупен
When("откроется форма редактирования орг структуры {string}", (text) => {
  I.see(text);
  I.wait(1);
});
// edit имя должности
When(
  "в поле имя должности с айди {string} введу текст {string}",
  (fieldName, value) => {
    I.fillField({ id: fieldName }, value);
    I.wait(1);
  }
);
//  кнопка Обновить orgstructure
...
// "Пом сис. Админа"
Then("увижу отредактированную должность {string}", (text) => {
