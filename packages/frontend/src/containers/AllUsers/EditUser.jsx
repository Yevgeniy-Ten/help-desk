...
const EditUser = () => {
  ...
  const { userForUpdate, isLoading } = useSelector(getUsersState, shallowEqual);
  ...
  const onChangeFields = (value) => {
    if (value.departmentId) {
      dispatch(
        fetchSettings("orgstructure", { departmentId: value.departmentId })
      );
    }
  };
  useEffect(() => {
    ...
    if (userForUpdate && userForUpdate.orgStructure) {
      dispatch(
        fetchSettings("orgstructure", {
          departmentId: userForUpdate.orgStructure.departmentId
        })
      );
    }
  }, [dispatch, id]);
