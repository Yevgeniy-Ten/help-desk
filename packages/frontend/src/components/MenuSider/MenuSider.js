...
import { FileTextOutlined, ContactsOutlined, DatabaseOutlined, SettingOutlined, HistoryOutlined, AuditOutlined } from "@ant-design/icons";
...
const MenuSider = () => {
    ...
    return (
        <Menu theme="dark" mode="inline">
            ...
            {user && user.role && user.role.name==="admin" && 
                <Menu.Item key="3" icon={<AuditOutlined style={{fontSize:"24px"}}/>}>
                    <NavLink to="/requests/audit">Аудит</NavLink>
                </Menu.Item>
            }
            {user && user.role && user.role.name==="admin" && 
                <Menu.Item key="4" icon={<ContactsOutlined style={{fontSize:"24px"}}/>}>
                    <NavLink to="/users">Контакты</NavLink>
                </Menu.Item>
            }
            <Menu.Item key="5" icon={<DatabaseOutlined style={{fontSize:"24px"}}/>}>
                <NavLink to="/faq">База знаний</NavLink>
            </Menu.Item>
            {user && user.role && user.role.name==="admin" && 
                <Menu.Item key="6" icon={<SettingOutlined style={{fontSize:"24px"}}/>}>
